...
void Toast(JNIEnv *env, jobject thiz, const char *text, int length) {
    if (env == nullptr || thiz == nullptr || text == nullptr) return;
    ...
    jmethodID showNativeToast = env->GetStaticMethodID(
            mainClass,
            OBFUSCATE("ShowNativeToast"),
            OBFUSCATE("(Landroid/content/Context;Ljava/lang/String;I)V"));
    if (showNativeToast == nullptr) {
        if (env->ExceptionCheck()) env->ExceptionClear();
    ...
    if (jstr != nullptr) {
        env->CallStaticVoidMethod(mainClass, showNativeToast, thiz, jstr, length);

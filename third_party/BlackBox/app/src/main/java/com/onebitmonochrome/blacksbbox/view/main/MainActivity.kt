...
class MainActivity : LoadingActivity() {
    ...
    private fun checkVpnPermission() {
        try {
            val vpnEnabled = try {
                AppManager.mBlackBoxLoader.useVpnNetwork()
            } catch (e: Exception) {
                Log.e(TAG, "Error reading VPN setting: ${e.message}")
                false
            }

            if (android.os.Build.VERSION.SDK_INT >= 34) {
                if (vpnEnabled) {
                    Log.w(TAG, "VPN setting enabled on SDK>=34; forcing disable (unsupported)")
                    try {
                        AppManager.mBlackBoxLoader.invalidUseVpnNetwork(false)
                    } catch (e: Exception) {
                        Log.e(TAG, "Error disabling VPN setting: ${e.message}")
                    }
                }
                return
            }
            ...
            if (!vpnEnabled) {
                Log.d(TAG, "VPN setting disabled; skipping VPN permission")
                return
            }

            val vpnIntent = VpnService.prepare(this)
            if (vpnIntent != null) {
                // Per UX requirement: only prompt for VPN permission when user explicitly
                // enables the toggle in Settings. If permission is missing at startup,
                // disable the toggle and let the user re-enable it to trigger the prompt.
                Log.w(TAG, "VPN setting enabled but permission missing; disabling setting")
                try {
                    AppManager.mBlackBoxLoader.invalidUseVpnNetwork(false)
                } catch (e: Exception) {
                    Log.e(TAG, "Error disabling VPN setting: ${e.message}")
    ...
    private fun initViewPager() {
        try {
            ...
            viewBinding.viewPager.setPageTransformer { page, position ->
                ...
                val scale = 0.94f + ((1f - absPos.coerceAtMost(1f)) * 0.06f)
                page.scaleX = scale
                page.scaleY = scale
                page.alpha = 0.75f + ((1f - absPos.coerceAtMost(1f)) * 0.25f)
                page.translationX = -position * 22f
            }
                    ...
                    object : ViewPager2.OnPageChangeCallback() {
                        override fun onPageSelected(position: Int) {
                            try {
                                ...
                                currentUser = if (position in userList.indices) {
                                    userList[position].id
                                } else {
                                    fragmentList[position].userID
                                }
                                updateUserRemark(currentUser)
                ...
                try {
                    if (it.resultCode == RESULT_OK) {
                        it.data?.let { data ->
                            ...
                            if (source != null) {
                                val targetFragment = fragmentList.firstOrNull { f -> f.userID == userId }
                                if (targetFragment != null) {
                                    targetFragment.installApk(source)
                                } else {
                                    Log.w(TAG, "No fragment found for userId=$userId; dropping install result")
                                }
                            }
                        }
                    }

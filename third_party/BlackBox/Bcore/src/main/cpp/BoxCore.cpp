...
void addIORule(JNIEnv *env, jclass clazz, jstring target_path,
    ...
    ALOGD("set addIORule");
    const char *targetPath = env->GetStringUTFChars(target_path, JNI_FALSE);
    const char *relocatePath = env->GetStringUTFChars(relocate_path, JNI_FALSE);
    IO::addRule(targetPath, relocatePath);
    env->ReleaseStringUTFChars(target_path, targetPath);

...
public class ActivityStack {
    ...
    private Intent startActivityProcess(int userId, Intent intent, ActivityInfo
        ...
        ActivityInfo launchInfo = resolveHostScreenOrientation(info);
        ProxyActivityRecord stubRecord = new ProxyActivityRecord(userId, launchInfo, intent, record);
        ...
        return getStartStubActivityIntentInner(intent, targetApp.bpid, userId, stubRecord, launchInfo);
    }

    /**
     * The Android system only knows about our ProxyActivity, so its manifest cannot carry the
     * target game's requested orientation. For host-cloned apps, use the real installed
     * ActivityInfo as the source of truth and carry its orientation into the virtual launch.
     */
    private ActivityInfo resolveHostScreenOrientation(ActivityInfo virtualInfo) {
        if (virtualInfo == null) {
            return null;
        }

        ActivityInfo result = new ActivityInfo(virtualInfo);
        try {
            ComponentName component = new ComponentName(virtualInfo.packageName, virtualInfo.name);
            ActivityInfo hostInfo = BlackBoxCore.getPackageManager().getActivityInfo(
                    component, PackageManager.GET_META_DATA);
            if (hostInfo != null) {
                result.screenOrientation = hostInfo.screenOrientation;
                Slog.i("BlackBoxOrientation", "Resolved " + component.flattenToShortString()
                        + " orientation virtual=" + virtualInfo.screenOrientation
                        + " host=" + hostInfo.screenOrientation);
            }
        } catch (Throwable error) {
            Slog.w("BlackBoxOrientation", "Could not resolve host orientation for "
                    + virtualInfo.packageName + "/" + virtualInfo.name + ": " + error.getMessage());
        }
        return result;
    }
